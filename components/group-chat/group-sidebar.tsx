    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-2 text-[10px] font-bold text-[#64748B]">
          <span className={previous ? "text-[#60A5FA]" : confirmed ? "text-[#34D399]" : "text-[#FBBF24]"}>{previous ? "Previous" : confirmed ? "Confirmed" : "Saved"}</span>
      ...
      <div className="space-y-4 px-3">
        {planSections.map((section) => (
          <section key={section.title} aria-labelledby={`${section.title}-heading`}>
            <h3 id={`${section.title}-heading`} className={`mb-2 px-2 text-[10px] font-black uppercase tracking-[0.14em] ${section.title === "Previous" ? "text-[#60A5FA]" : section.title === "Confirmed" ? "text-[#34D399]" : "text-[#FBBF24]"}`}>{section.title}</h3>
